        ...
        <div className="w-full bg-slate-900 rounded-lg border border-slate-800 overflow-hidden shadow-2xl h-80 flex flex-col">
          ...
          <div className="p-4 overflow-y-auto font-mono text-sm space-y-2 flex-1 scrollbar-hide">
            {log.map((line, i) => (
              <div
                key={i}
                className={`${line.includes('TARGET') ? 'text-red-400 font-bold' : line.includes('Tracking') ? 'text-yellow-400' : 'text-green-400'}`}
              >
        ...
        >
          ...
          >
            <div className="relative">
              <div
                className={`w-14 h-14 rounded-full border-2 ${step === 1 ? 'border-red-500 bg-red-900/20' : 'border-slate-600 bg-slate-900'} flex items-center justify-center overflow-hidden transition-colors duration-500`}
              >
                <Globe
                  size={32}
                  className={`${step === 1 ? 'text-red-400' : 'text-slate-500'} opacity-80`}
                />
              </div>
              <MapPin
                size={24}
                className={`${step === 1 ? 'text-red-500' : 'text-yellow-500'} absolute -top-1 -right-1 animate-bounce`}
                fill="currentColor"
              />
            </div>
            <div className="min-w-[140px]">
              ...
              <div
                className={`font-bold text-lg transition-colors duration-300 ${step === 1 ? 'text-red-400' : 'text-white'}`}
              >
                {currentLocation}
              </div>
            </div>
