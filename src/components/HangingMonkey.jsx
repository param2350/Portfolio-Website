...
const HangingMonkey = ({ onClick, isFooterLocked }) => {
    ...
    const [lifecycle, setLifecycle] = useState('docked'); // Always docked now
    ...
    }, [lifecycle, isFleeing, isFooterLocked, isHovered]);
    ...
    }, [isVisible, isFleeing, side, isHovered]);
    ...
    // Handlers
    const handleMouseEnter = () => { setIsHovered(true); setSpeech("System Override!"); };
    const handleClick = () => { setSpeech("Accessing Mainframe..."); setTimeout(onClick, 500); };

    // Dynamic Styles
    const getContainerStyle = () => {
        const base = { right: side === 'right' ? '2rem' : 'auto', left: side === 'left' ? '2rem' : 'auto' };
        ...
        // 2. Fleeing State
        if (!isVisible && isFleeing) {
            if (evasionMode === 'drop') return { ...base, top: 0, transform: 'translateY(110vh) rotate(20deg)' };
            return { ...base, top: 0, transform: 'translateY(-150%)' };
        }
    ...
    return (
        ...
        >
            ...
            ${isHovered || isFleeing ? 'scale-100 opacity-100' : 'scale-75 opacity-0 pointer-events-none'}`}
                ...
                <div className={`absolute -top-[100px] left-1/2 -translate-x-1/2 w-1 h-[100px] bg-slate-700 transition-opacity duration-300 ${evasionMode === 'drop' && isFleeing ? 'opacity-0' : 'opacity-100'}`}></div>
                ...
                >
                    ...
                    <div
                        ...
                        style={{ backgroundColor: dangerLevel > 0.5 ? '#7f1d1d' : '#1e293b' }}
                    ></div>

                    {/* Cyber Visor */}
                    <div className={`absolute top-[22px] left-2 right-2 h-4 bg-cyan-500/80 rounded-sm z-20 blur-[1px] transition-opacity duration-200 ${isHovered ? 'opacity-100' : 'opacity-0'}`}></div>

                    {/* Eyes Container */}
                    <div className="flex gap-2 relative z-10 mt-1">
                        {/* Eyes */}
                        {[0, 1].map(i => (
                            <div key={i} className="w-4 h-4 bg-black rounded-full relative overflow-hidden ring-1 ring-slate-600">
                                <div
                    ...
                    <div
                        ...
                        style={{
                            transform: dangerLevel > 0.7 ? 'rotate(10deg) scale(0.8)' : 'rotate(0deg)',
